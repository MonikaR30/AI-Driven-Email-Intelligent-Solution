...
# ── PROMO DETECTION LISTS ──────────────────────────────────────────────────────
PROMO_DOMAINS = [
    "naukri.com", "hackerrank.com", "internshala.com", "linkedin.com",
    "newsletter", "mailer", "no-reply", "noreply", "donotreply",
    "info@", "hello@", "team@", "support@", "notification@",
    "amazon.com", "flipkart.com", "swiggy.com", "zomato.com",
    "udemy.com", "coursera.org", "geeksforgeeks.org", "leetcode.com"
]

PROMO_SUBJECT_KEYWORDS = [
    "top companies hiring", "jobs for you", "new jobs matching",
    "improve your coding", "challenge", "contest", "newsletter",
    "unsubscribe", "digest", "weekly", "daily update",
    "sale", "offer", "discount", "deal", "% off", "free trial",
    "student partner", "rewards", "win a", "ps5", "airwrap",
    "application status has changed"
]

SPAM_KEYWORDS = [
    "you won", "winner", "lottery", "prize", "claim now",
...
{{
  "intent": "<one of: Meeting | Request | Update | Notification | Promotion | OTP | Job | Personal | Security>",
...
- Security   : security alerts, suspicious login, account warnings from Google/Microsoft/banks
- Meeting    : scheduling, calendar invites, interview time slots
- Request    : sender asks YOU to do something specific (submit, complete, review)
- Job        : job offers, recruitment, application status from HR/companies
- Promotion  : marketing emails, newsletters, course ads, platform promotions
               (Naukri, LinkedIn jobs digest, HackerRank challenges, Internshala campaigns,
                Amazon/Flipkart deals, any "top companies hiring" digest = Promotion)
- Update     : status updates, progress reports, announcements
- Notification: system alerts, confirmations, receipts
- Personal   : friends, family, personal conversations

=== CRITICAL PRIORITY RULES ===
Promotion emails (Naukri digests, job alerts newsletters, HackerRank challenges,
Internshala campaigns, course ads) are NEVER high priority — they are LOW.

Only mark as HIGH priority if the email:
- Contains a DEADLINE you must act on TODAY or within 48 hours
- Is a direct personal message requiring urgent action
- Is a real security threat (unauthorized login, account compromised)
- Is a real job offer or interview invite with a deadline

=== REPLY RULES ===
- Body only — no "Hi", no "Thanks", no signature
- For Promotion, OTP, Security, Notification intents → set reply to "No reply required."
...
"""
    ...
    try:
        ...
        valid_intents = ["Meeting", "Request", "Update", "Notification",
                         "Promotion", "OTP", "Job", "Personal", "Security"]
...
def detect_priority(text: str, intent: str, subject: str = "") -> str:
    ...
    if intent in ["Promotion", "OTP", "Notification"]:
        return "LOW"

    # Promo domain check
    if any(domain in combined for domain in PROMO_DOMAINS):
        return "LOW"

    # Promo subject keyword check
    if any(kw in subject_lower for kw in PROMO_SUBJECT_KEYWORDS):
        return "LOW"
    ...
    if intent in ["Meeting", "Request", "Security"]:
        return "HIGH"
...
def analyze():
    ...
    try:
        ...
        # ── INTENT → CATEGORY MAPPING (overrides classifier) ──────────────────
        intent_to_category = {
            "OTP":          "OTP",
            "Security":     "Other",
            "Meeting":      "Meeting",
            "Job":          "Job",
            "Personal":     "Personal",
            "Promotion":    "Other",      # newsletters/promos go to Other, NOT Spam
            "Request":      "Work",
            "Update":       "Work",
            "Notification": "Other",
        }
        ...
        # ── SPAM OVERRIDE — only real scam emails go to Spam ──────────────────
        if any(kw in subject.lower() for kw in SPAM_KEYWORDS):
            category = "Spam"
            print(f"🚫 Category overridden to Spam based on subject keywords")
